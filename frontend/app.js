...
/* =========================================================
   INTEGRITY WARNING PANEL + "GEMA misunderstood?" OVERRIDE
   ========================================================= */
...
function renderIntegrityWarning(body) {
    ...
    $("integrityWarningRules").innerHTML = body.per_rule.map(r => {
        const flagged = body.outlier_rule_ids.includes(r.rule_id);
        return `
            <div class="integrity-card ${flagged ? "flagged" : ""}">
                <span class="integrity-card-id">${escapeHtml(r.rule_id)}${flagged ? " · FLAGGED" : ""}</span>
                <span class="integrity-card-rate">${r.pass_rate_pct}% pass rate</span>
                <span class="integrity-card-clause">${escapeHtml(r.clause_text)}</span>
            </div>
        `;
    }).join("");

    $("integrityWarningPrecedents").innerHTML = body.similar_precedents.length
        ? `<div class="integrity-precedents-label">SIMILAR PAST CASES</div>` +
          body.similar_precedents.map(p => `
              <div class="source-quote">
                  <strong>${escapeHtml(p.kind)}</strong> by ${escapeHtml(p.actor)} (similarity ${p.similarity}, ${escapeHtml(p.method)}):
                  "${escapeHtml(p.reason)}"
              </div>
          `).join("")
        : "";

    // A fresh warning means any earlier override's Annexure B no longer
    // applies to THIS pending decision -- hide it until this one is
    // resolved, so the officer can't download a stale document by mistake.
    $("downloadAnnexureBButton").classList.add("hidden");
...
$("integrityOverrideButton").addEventListener("click", async () => {
    ...
    try {
        ...
        // The override is now logged in the tender-integrity ledger, so
        // Annexure B exists -- surface it immediately instead of leaving
        // the officer to hunt for a download link.
        $("downloadAnnexureBButton").classList.remove("hidden");
        downloadIntegrityAnnexure();
...
// "Annexure A" -- the existing per-bidder rejection justification PDF.
// The download button in Bidder Comparison called this by name, but it
// was never actually defined, so the button silently did nothing.
window.downloadAnnexure = function () {
    const bidderId = $("reportBidderSelect")?.value;
    if (!bidderId) {
        toast("Select a bidder and generate a report first.");
        return;
    }
    window.open(`${API_BASE}/api/v3/bidders/${bidderId}/rejection-annexure`, "_blank");
};
...
/* =========================================================
   TENDER INTEGRITY — Restrictiveness + Outlier checks
   (Risk & Impact page, "Publish-time check" card)
   ========================================================= */

async function runRestrictivenessCheck() {
    try {
        const result = await api("/api/v3/tender/restrictiveness");
        renderTenderIntegrityResult(result, "restrictiveness");
    } catch (error) {
        toast(`Could not run restrictiveness check: ${error.message}`);
    }
}

async function runOutlierCheck() {
    try {
        const result = await api("/api/v3/tender/outlier-rules");
        renderTenderIntegrityResult(result, "outlier");
    } catch (error) {
        toast(`Could not run outlier check: ${error.message}`);
    }
}

function renderTenderIntegrityResult(result, kind) {
    const box = $("restrictivenessResult");

    if (kind === "restrictiveness") {
        box.innerHTML = `
            <div class="integrity-verdict ${result.verdict}">
                <span class="verdict-label">${escapeHtml(result.verdict.replace(/_/g, " "))}</span>
                <span class="verdict-rate">${result.mandatory_pass_rate_pct}% of the synthetic population qualifies</span>
            </div>
            <p class="integrity-message">${escapeHtml(result.message)}</p>
            <p class="integrity-note">${escapeHtml(result.note)}</p>
        `;
        return;
    }

    const rows = result.per_rule.map(r => `
        <div class="integrity-check-row ${r.outlier ? "outlier" : ""}">
            <div class="check-id">${escapeHtml(r.rule_id)}${r.outlier ? " · FLAGGED" : ""}</div>
            <div class="check-rate">${r.pass_rate_pct}%</div>
            <div class="check-clause">${escapeHtml(r.clause_text)}</div>
        </div>
    `).join("");

    box.innerHTML = `
        <div class="integrity-verdict ${result.has_outlier ? "FLAGGED" : "NORMAL"}">
            <span class="verdict-label">${result.has_outlier ? "Outlier rule detected" : "No outliers detected"}</span>
            <span class="verdict-rate">${result.sample_size} synthetic profiles tested</span>
        </div>
        ${rows}
    `;
}

$("restrictivenessButton").addEventListener("click", runRestrictivenessCheck);
$("outlierButton").addEventListener("click", runOutlierCheck);
...
function loadDemoState() {
    ...
    state.rules = [
        {
            rule_id: "R42",
            clause_text: "Turnover must be greater than or equal to Rs.10 Cr",
            weight: 50,
            is_mandatory: true
        },
        {
