...
from restrictiveness import evaluate_tender_restrictiveness, identify_outlier_rules
from legal_defense import generate_rejection_annexure
...
def clear_audit_session() -> dict:
    ...
    global active_bidder_id, current_tender_rules, current_tender_deadline, current_tender_filename, current_tender_department, current_tender_contract_value
...
async def ingest_documents(
    ...
    if tender_payload is not None:
        ...
        # Pull estimated_contract_value out of the tender's own extracted
        # facts, if Gemini found one -- powers the Restrictiveness Index
        # and Outlier Rule checks, which need to know the project's scale
        # to build a realistic reference population instead of a generic
        # nationwide one.
        current_tender_contract_value = next(
            (
                float(fact["value"])
