...
# ----------------------------------------------------------------------
# TENDERS — keyed so the same ingest flow can recognise EITHER the real
# honest tender OR the constructed corrupted demo tender, purely by
# filename. Add more entries here the same way for any future demo tender.
# ----------------------------------------------------------------------
...
_NASHIK_REQUIREMENTS = [
    {
        "description": "Average annual turnover of at least Rs.100 Crore in the last three financial years",
        "op": ">=",
...
# ---- The corrupted demo tender (Dhule) -------------------------------
# Identical structure to the honest Nashik tender EXCEPT rule 1 -- the
# turnover requirement is replaced with a suspiciously narrow band
# (>=137 Cr AND <=140 Cr) instead of a simple floor, matching the real
# PDF in CORRUPTED_TENDER_DEMO_Dhule.pdf. Every other clause (experience,
# PAN, GSTIN, Udyam, EMD) is left completely normal on purpose -- that
# contrast is the entire point of the demo.

_DHULE_DOCUMENT = {
    "filename": "CORRUPTED_TENDER_DEMO_Dhule.pdf",
    "document_type": "tender_notice",
    "bidder_name_on_document": None,
    "facts": [
        {
            ...
            "page": 1,
            "valid_until": None,
            "source_quote": "Estimated Contract Value Rs. 95,00,00,000 (Rupees Ninety-Five Crore only).",
        },
        {
            "entity_name": "bid_opening_date",
            "value": "2026-10-21",
            ...
            "page": 1,
            "valid_until": None,
            "source_quote": "Bid Opening Date 21 October 2026, 11:00 hrs.",
        },
    ],
    "notes": (
        "CONSTRUCTED DEMO DOCUMENT -- illustrative only, not a real tender. "
        "Prepared for SIH 2026 to demonstrate the Outlier Rule detector."
    ),
}

_DHULE_REQUIREMENTS = [
    {
        # <-- the planted clause: a narrow AND-band instead of a simple
        # floor, tight enough that almost no real firm sits inside it.
        "description": "Average annual turnover not less than Rs.137 Crore and not more than Rs.140 Crore in the last three financial years",
        "op": "AND",
        "entity_name": None,
        "value": None,
        "mandatory": True,
        "weight": 10.0,
        "children": [
            {"description": None, "op": ">=", "entity_name": "turnover", "value": "1370000000", "mandatory": True, "weight": 10.0, "children": None},
            {"description": None, "op": "<=", "entity_name": "turnover", "value": "1400000000", "mandatory": True, "weight": 10.0, "children": None},
        ],
    },
    {
        "description": "Technical Experience Requirements",
    ...
    {
        "description": "Bid Security / EMD Bank Guarantee valid until at least 18 January 2027",
        "op": "DATE_AFTER",
        "entity_name": "emd_valid_until",
        "value": "2027-01-18",
        "mandatory": True,
...
TENDERS = {
    "nashik": {
        "match": ["nashik", "8847213"],
        "document": _NASHIK_DOCUMENT,
        "requirements": _NASHIK_REQUIREMENTS,
        "closing_date": "2026-09-30",
    },
    "dhule_corrupted": {
        "match": ["dhule", "corrupted", "8847299"],
        "document": _DHULE_DOCUMENT,
        "requirements": _DHULE_REQUIREMENTS,
        "closing_date": "2026-10-20",
    },
}

# Back-compat aliases (in case any code imports these names directly).
TENDER_CLOSING_DATE = TENDERS["nashik"]["closing_date"]
TENDER_DOCUMENT = _NASHIK_DOCUMENT
TENDER_REQUIREMENTS = _NASHIK_REQUIREMENTS
...
def _match_tender_key(filename: str):
    name = filename.lower()
    for key, entry in TENDERS.items():
        if any(token in name for token in entry["match"]):
            return key
    # Generic fallback tokens so a differently-named honest tender file
    # still resolves to *something* recognisable rather than silently
    # falling through to a live Gemini call.
    if any(token in name for token in ("tender", "hospital")):
        return "nashik"
    return None
...
def try_fixture_extraction(bidder_payload, tender_payload):
    ...
    if tender_payload is not None:
        tender_key = _match_tender_key(tender_payload["filename"])
        if tender_key is None:
    ...
    if tender_key is not None:
        documents.append(dict(TENDERS[tender_key]["document"]))
    for key in bidder_keys:
        documents.extend(BIDDER_DOCUMENTS[key]["documents"])

    return {
        "documents": documents,
        "requirements": list(TENDERS[tender_key]["requirements"]) if tender_key is not None else [],
        "tender_closing_date": TENDERS[tender_key]["closing_date"] if tender_key is not None else None,
        "extraction_model": "demo-fixture",
    }


# ----------------------------------------------------------------------
# Expected results, Nashik tender (weight=10.0 x 6 rules -> total 60):
#
...
#
# Dhule (corrupted) tender, same 3 bidders, run through
# /api/v3/tender/outlier-rules -- outlier_rule_ids should contain
# exactly the planted turnover-band rule and nothing else.
# ----------------------------------------------------------------------
